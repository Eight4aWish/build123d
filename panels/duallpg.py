"""DualLPG — N8Synth 10HP 3x6 panel, dual low-pass-gate layout.

Derived from `CortHex.py` (same SVG-extracted 3x6 hole grid) with the
DualLPG-specific changes:

  - Panel width corrected to the Doepfer 10HP cut width (50.5 mm) instead of
    the nominal 50.8 mm pitch, so modules don't bind in a populated row.
  - Hole (column, row) = (3, 6) removed (grid index 17, bottom-right).
  - Holes (2, 1) and (2, 4) enlarged to 7.5 mm to accept pots (the pot-hole
    size used on the mkikick module).
  - All content (holes + labels) shifted 2 mm up via `content_y_offset` so the
    bottom row clears the rack rail; the mounting slots stay put.
  - No LEDs (CortHex's column-2 LED combos dropped entirely).
  - Pot labels sit a touch lower than jack labels to clear the wider opening.

Run:
  ./.venv/bin/python duallpg.py

Export:
  ./.venv/bin/python duallpg.py --export-mode base   --stl-base duallpg_base.stl
  ./.venv/bin/python duallpg.py --export-mode labels --stl-labels duallpg_labels.stl

Paper template (1:1):
  ./.venv/bin/python duallpg.py --template-svg duallpg_template.svg
"""
...
class FaceplateParams:
    ...
    # Match the rest of the range's orientation.
    flip_y: bool = True
    ...
    # Control holes.
    hole_d: float = _u2mm(2 * 1358.627)  # from SVG ellipse rx, ~6.90 mm
    hole_oversize: float = 0.0

    # Pot holes — enlarged openings (mkikick pot size). Each entry is an index
    # into `holes_sorted`. (2,1)=idx1 and (2,4)=idx10.
    pot_hole_indices: tuple[int, ...] = (1, 10)
    pot_hole_d: float = 7.5
    ...
    # left-to-right). PLACEHOLDERS — set these to the real DualLPG functions.
    hole_labels_below: tuple[str, ...] = (
        # Row 1 (top):  col1    col2(pot)  col3
        "IN 1",        "DEPTH",  "OUT 1",
        # Row 2:
        "ATTEN",      "MANUAL",  "",
        # Row 3:
        "CV",            "STRIKE",        "MIX",
        # Row 4:        col1      col2(pot)  col3
        "IN 2",        "DEPTH",  "OUT 2",
        # Row 5:
        "ATTEN",      "MANUAL",  "",
        # Row 6 (bottom): col1    col2       (col3 removed)
        "CV",            "STRIKE",        "",
    )
    ...
    # Remove hole (3,6) = grid index 17 (bottom-right).
    remove_hole_indices: tuple[int, ...] = (17,)
    ...
    def hole_diameter_for(self, idx: int) -> float:
        return self.pot_hole_d if idx in set(self.pot_hole_indices) else self.hole_d
...
def build_base(params: FaceplateParams) -> object:
    ...
    pots = set(int(i) for i in params.pot_hole_indices)
    ...
    with BuildPart() as p:
        ...
        with BuildSketch(Plane.XY.offset(cut_z0)):
            ...
            for idx, (hx, hy) in enumerate(holes_all):
                if idx in disabled:
                    continue
                d = params.pot_hole_d if idx in pots else params.hole_d
                with Locations((hx, hy)):
                    Circle((d + params.hole_oversize) / 2)
...
def build_labels(params: FaceplateParams) -> object | None:
    ...
    with BuildPart() as p:
        ...
        for idx, (hx, hy) in enumerate(holes_all):
            if idx in disabled:
                continue
            # Pots get a little extra downward offset to clear the wider hole.
            extra = params.pot_label_extra_dy if idx in pots else 0.0
            if idx < len(params.hole_labels_below):
                txt = params.hole_labels_below[idx].strip()
                if txt:
                    add_text_extrusion(txt, x=hx + dx_off, y=hy + dy_off + extra, rot=0.0, font_size=params.label_size)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build DualLPG 10HP 3x6 Eurorack panel")
