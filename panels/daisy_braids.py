"""DaisyBraids 10HP OLED faceplate - Face-up 2-color printing.

Braids macro-oscillator firmware on a Daisy Patch Init, with OLED screen, SD
card slot and two screen mounting bolts. Shares the Daisy Patch Init hardware
base; relabel HOLE_LABELS for sibling builds (e.g. DaisyMFX).
...
- OLED screen cutout (14mm x 11.5mm) with mounting holes

The panel prints FACE-UP with embossed labels on the top surface.

Run:
  ./.venv/bin/python daisybraids.py

Export STLs:
  ./.venv/bin/python daisybraids.py --stl-base base.stl --stl-labels labels.stl
...
# Note: B8 hole removed for OLED screen
HOLE_LABELS = (
    "OUT-R",# OUT-R
    "OUT-L",# OUT-L
    "",# IN-R
...
# Note: B8 hole at (25.502894, 61.956597) removed for OLED screen
# ---------------------------------------------------------------------------
# OLED Y Offset (for easy adjustment)
# ---------------------------------------------------------------------------
OLED_Y_OFFSET = -3  # Change this value to move OLED cutout and mounting holes up/down
HOLES: tuple[Hole, ...] = (
    ...
    # B8 removed: Hole(shape="circle", x=25.502894, y=61.956597, drill=(6.2, 6.2)),
    ...
    # OLED mounting holes (3mm diameter, 19.5mm apart, centered 2.5mm down from screen top, y offset adjustable)
    # OLED mounting holes shifted 0.5mm further from screen center line
    Hole(shape="circle", x=15.65, y=64.75 + OLED_Y_OFFSET, drill=(3.0, 3.0)),  # Left mounting hole
    Hole(shape="circle", x=35.15, y=64.75 + OLED_Y_OFFSET, drill=(3.0, 3.0)),  # Right mounting hole
)
...
# Rectangular cutouts including OLED screen window
# OLED screen: 14mm wide x 11.5mm high, centered at panel X center (25.4mm)
# Screen center Y at 62mm (approximately where B8 was)
RECT_CUTOUTS: tuple[RectCutout, ...] = (
    RectCutout(x=24.14, y=33.493, w=27.348 - 24.14, h=46.295 - 33.493),
    # OLED screen cutout: 14mm x 11.5mm centered at (25.4, 62 + OLED_Y_OFFSET)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="DaisyBraids 10HP OLED faceplate")
