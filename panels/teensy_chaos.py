...
class FaceplateParams:
	...
	# Remove/disable holes (sorted top-to-bottom, left-to-right) to make room for the OLED.
	# 0,1 = row1 col1/col2 (blank holes); 3,4 = row2 col1/col2 (GATE, ROOT)
	remove_hole_indices: tuple[int, ...] = (0, 1, 3, 4)

	# Which two holes define the OLED placement area (by sorted index).
	# Anchored at top-left (0) and row2-col2 (4) to centre the screen in the 2×2 cleared area.
	screen_anchor_indices: tuple[int, int] = (0, 4)
	...
	# Centre-to-centre spacing of the 4 mount holes (square pattern)
	screen_mount_spacing: float = 23.5
	# Distance from top edge of screen cutout to centre of top mount holes
	screen_top_gap: float = 3.2
	...
	def screen_geometry(self) -> tuple[tuple[float, float], list[tuple[float, float]]]:
		...
		Horizontal: screen centred on the X midpoint of the two anchor holes.
		Vertical: bottom mount holes aligned to the Y of screen_bottom_hole_align_index;
		screen position and top mount holes are derived from that anchor + screen_top_gap.
		"""

		holes = self.holes_sorted()

		a0, a1 = self.screen_anchor_indices
		screen_cx = (holes[int(a0)][0] + holes[int(a1)][0]) / 2

		# Derive screen_cy from the bottom mount hole alignment:
		#   bottom_holes_y  = holes[align_index].y
		#   mount_cy        = bottom_holes_y + half
		#   mount_cy        = screen_cy + screen_h/2 + screen_top_gap - half
		#   => screen_cy    = bottom_holes_y + screen_mount_spacing - screen_h/2 - screen_top_gap
		half = self.screen_mount_spacing / 2
		bottom_holes_y = holes[self.screen_bottom_hole_align_index][1]
		screen_cy = bottom_holes_y + self.screen_mount_spacing - self.screen_h / 2 - self.screen_top_gap

		mount_cy = screen_cy + self.screen_h / 2 + self.screen_top_gap - half
		mount_pts = [
			(screen_cx - half, mount_cy - half),
			(screen_cx + half, mount_cy - half),
			(screen_cx - half, mount_cy + half),
			(screen_cx + half, mount_cy + half),
		]
